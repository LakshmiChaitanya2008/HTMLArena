...
export default function Navbar() {
  ...
  const [signIn, user, loading, error] = useSignInWithGithub(auth);
  const [cuUser] = useAuthState(auth);
      ...
      <div className="container mx-auto flex flex-wrap justify-between p-2 flex-row items-center">
        ...
        <div>
          ...
          {!cuUser ? (
            <button
              type="button"
              className="text-white bg-[#24292F] hover:bg-[#24292F]/90 focus:ring-4 focus:ring-[#24292F]/50 font-medium rounded-lg px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-500 dark:hover:bg-[#050708]/30 mr-2 mb-2"
            >
              <svg
              ...
              <span onClick={async () => await signIn()}>
                Sign in with Github
              </span>
            </button>
          ) : (
            <button className="inline-flex items-center border-0   px-3  rounded text-xl mb-1">
              <img src={cuUser.photoURL} className="w-7 rounded-full" />
            </button>
          )}
        </div>
      </div>
